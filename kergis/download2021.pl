...
system("echo 'create table kergis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Kerala\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE kergis ADD COLUMN booth_id_21 INTEGER;' > kergis2021.sql");
system("echo '.dump kergis' | sqlite3 electoralsearch.in.sqlite >> kergis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' kergis2021.sql");
