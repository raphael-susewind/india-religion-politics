...
# Extract the three data columns and re-load the respective data, putting it all into @raw, deleting temp files
    system("pdftotext -nopgbrk -f 2 -l $pages  -x 30 -y 80 -W 190 -H 1020 -layout -r 100 $file out1.txt");
    system("pdftotext -nopgbrk -f 2 -l $pages  -x 280 -y 80 -W 190 -H 1020 -layout -r 100 $file out2.txt");
    system("pdftotext -nopgbrk -f 2 -l $pages  -x 530 -y 80 -W 190 -H 1020 -layout -r 100 $file out3.txt");
...
# Create rolls.X.sqlite to put in all the necessary stuff
    ...
    $dbh_rolls->do ("CREATE TABLE rolls (id INTEGER PRIMARY KEY AUTOINCREMENT, constituency INTEGER, booth INTEGER, rollno INTEGER, voterid CHAR, nameparts INTEGER, age INTEGER, gender CHAR, community CHAR, certainty FLOAT, gap FLOAT, name CHAR, fathername CHAR, voter_nameparts INTEGER, voter_community CHAR, voter_certainty FLOAT, voter_gap FLOAT, father_nameparts INTEGER, father_community CHAR, father_certainty FLOAT, father_gap FLOAT, ngram INTEGER)");
