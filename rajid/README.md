...
This table contains the ID matching and integration table for Rajasthan. Each row is the equivalent of a booth. Since the databse only contains data from one year for Rajasthan so far, this is rather straightforward - no actual cross-year matching is necessary.
