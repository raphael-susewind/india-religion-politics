...
system("echo 'create table hargis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Haryana\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE hargis ADD COLUMN booth_id_21 INTEGER;' > hargis2021.sql");
system("echo '.dump hargis' | sqlite3 electoralsearch.in.sqlite >> hargis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' hargis2021.sql");
