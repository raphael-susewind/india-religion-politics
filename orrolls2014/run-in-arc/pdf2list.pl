...
sub devanagarilast {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_or = ? AND namepart = 'l' GROUP BY community");
    $sth->execute($_[0]);
...
# match against devanagari first name_or gendered male
sub devanagarifirstm {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_or = ? AND namepart = 'f' AND gender = 'm' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
# match against devanagari first name_or gendered female
sub devanagarifirstf {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_or = ? AND namepart = 'f' AND gender = 'f' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
sub devanagarifirst {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_or = ? AND namepart = 'f' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
for ($page=1;$page<$pagecount;$page++) {
    $xml .= `PYTHONPATH=/home/area-mnni/rsusewind/lib/python2.6/site-packages:/system/software/linux-x86_64/lib/python2.6/site-packages python2.6 /home/area-mnni/rsusewind/bin/pdf-table-extract -i $file -p $page -r 300 -l 0.9 -t cells_xml`;
}
...
foreach my $cell (@xml) {
    ...
    my $gender='';
    if ($cell =~ /ପର\s+$/) {$gender='m'} else {$gender='f'}
    ...
    system("tiffcrop -U px -z 184,38,540,186 $const-$booth-$rollno.tif $const-$booth-$rollno-ocr.tif");
...
#    unless ($shades > 10) { # not pure black & white, i.e. "deleted" overlay or some such
	...
	my $temp= `tesseract -psm 4 -l ori --tessdata-dir /home/area-mnni/rsusewind/share/tessdata $const-$booth-$rollno-ocr.tif stdout`;
	$temp =~ s/\n/ /gs;
        $temp =~ s/[^\p{Oriya}\s]//gs;
