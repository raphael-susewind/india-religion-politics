...
system("sqlite3 temp.sqlite '.dump harrolls2021' > harrolls2021-a.sql");

open (FILE, ">>harrolls2021-a.sql");

print FILE ".header on\n";
print FILE ".mode csv\n";
print FILE ".once harrolls2021/harrolls2021.csv\n";
print FILE "SELECT * FROM harrolls2021;\n";

close (FILE);

system("sqlite3 temp.sqlite '.dump harid' > harrolls2021-b.sql");

open (FILE, "harrolls2021-b.sql");
my @file = <FILE>;
close (FILE);

open (FILE, ">harrolls2021-b.sql");

print FILE "ALTER TABLE harid ADD COLUMN booth_id_21 INTEGER;\n";

foreach my $line (@file) {
    if ($line =~ /^INSERT INTO harid/) {$line =~ s/^INSERT INTO harid/INSERT INTO harid (ac_id_09, booth_id_21, booth_id_14)/; $line =~ s/'unclear'/NULL/gs; print FILE $line;}
}
