...
system("echo 'create table mpgis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Madhya Pradesh\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE mpgis ADD COLUMN booth_id_21 INTEGER;' > mpgis2021.sql");
system("echo '.dump mpgis' | sqlite3 electoralsearch.in.sqlite >> mpgis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' mpgis2021.sql");
