...
system("echo 'create table gujgis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Gujarat\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE gujgis ADD COLUMN booth_id_21 INTEGER;' > gujgis2021.sql");
system("echo '.dump gujgis' | sqlite3 electoralsearch.in.sqlite >> gujgis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' gujgis2021.sql");
