...
system("echo 'create table orgis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Odisha\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE orgis ADD COLUMN booth_id_21 INTEGER;' > orgis2021.sql");
system("echo '.dump orgis' | sqlite3 electoralsearch.in.sqlite >> orgis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' orgis2021.sql");
