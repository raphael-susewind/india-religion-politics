...
system("echo 'create table upgis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Uttar Pradesh\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE upgis ADD COLUMN booth_id_21 INTEGER;' > upgis2021.sql");
system("echo '.dump upgis' | sqlite3 electoralsearch.in.sqlite >> upgis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' upgis2021.sql");
