...
system("echo 'create table rajgis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Rajasthan\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE rajgis ADD COLUMN booth_id_21 INTEGER;' > rajgis2021.sql");
system("echo '.dump rajgis' | sqlite3 electoralsearch.in.sqlite >> rajgis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' rajgis2021.sql");
