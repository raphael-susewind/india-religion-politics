...
for ($ac=1;$ac<=90;$ac++) {
    ...
    print FILE "UPDATE harid SET ac_name_14 = '".@$ref[0]."' WHERE ac_id_09 = $ac;\n";
    my $ref = $dbh->selectcol_arrayref("SELECT ac_reserved FROM actopc WHERE ac = ?",undef,$ac);
    print FILE "UPDATE harid SET ac_reserved_14 = '".@$ref[0]."' WHERE ac_id_09 = $ac;\n";
