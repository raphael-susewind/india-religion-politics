...
foreach my $file (@ARGV) {
	...
	my ($community,$certainty) = checkname($fields[3]);
	if ($fields[5] ne 'GEN' && $community eq 'Muslim') {$community = 'Unknown'; $certainty = 0}
	if ($fields[5] ne 'GEN' && $community eq 'Christian') {$community = 'Unknown'; $certainty = 0}
	open (FILE, ">>$file");
 	print FILE join(",",@fields).",$community,$certainty\n";
	close (FILE);
	print $fields[3]." - $community\n";
    }
