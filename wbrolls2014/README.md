...
Prof Atanu Biswas of the Indian Statistical Institute in Kolkata was kind enough to manually classify a sample of 1000 random names by religion (Muslim / non-Muslim). This allowed me to estimate the accuracy of my automated approach, and unfortunately it is much less accurate than with names from UP written in Devanagari (as reported  [here](https://github.com/raphael-susewind/name2community)). Specifically, the algorithm in this case (Bengali script and WB names) only achieves a sensitivity and specificity of 0.69 and 0.70, respectively, with positive predictive value of 0.44 and negative predictive value of 0.87 (see accuracy-sample.csv and accuracy-sample.pl).  In consequence, this table over-estimates Muslim demography by quite a bit, so tread carefully: relative comparisons between booths, ACs, etc should be meaningful, but absolute figures are wrong. You have been warned!

**Correction, 22 September 2026:** until this date, this folder also contained two files, random-names.csv and random-names-pure.csv, each listing 1,000 sampled electors by name and relative's name, with their inferred religion (and, in the first file, the manual classification). They were the accuracy sample described above. As personal data, they should not have been public. They were added in June 2017 and removed on 22 September 2026, and purged from the entire history of this repository at the same time. Only the aggregate counts needed to reproduce the accuracy figures are kept, in accuracy-sample.csv. Copies of this repository made before 22 September 2026, including forks, may still contain the files; please delete them if you hold such a copy.
