...
$dbh->do ("CREATE TABLE gujcandidates2014 (id INTEGER PRIMARY KEY,pc_id_14 INTEGER)");

foreach my $key (keys(%headersql)) {
    $dbh->do ("ALTER TABLE gujcandidates2014 ADD COLUMN candidate_".$key."_name_14 CHAR");
    $dbh->do ("ALTER TABLE gujcandidates2014 ADD COLUMN candidate_".$key."_religion_14 CHAR");
    $dbh->do ("ALTER TABLE gujcandidates2014 ADD COLUMN candidate_".$key."_religion_certainty_14 INTEGER");
}
...
while (my $line = shift(@in2)) {
    ...
    if ($oldconst == $pc) {
	$dbh->do("UPDATE gujcandidates2014 SET candidate_".$party."_name_14 = ?, candidate_".$party."_religion_14 = ?, candidate_".$party."_religion_certainty_14 = ? WHERE pc_id_14 = ?",undef,$columns[1],$columns[3],$columns[4],$pc);
    } else { 
	$dbh->do("INSERT INTO gujcandidates2014 (pc_id_14, candidate_".$party."_name_14, candidate_".$party."_religion_14, candidate_".$party."_religion_certainty_14) VALUES (?,?,?,?)",undef,$pc,$columns[1],$columns[3],$columns[4]);
	$oldconst=$pc;
    }
}
...
system("sqlite3 temp.sqlite '.dump gujcandidates2014' > gujcandidates2014.sql");

open (FILE, ">>gujcandidates2014.sql");

print FILE ".mode csv\n";
print FILE ".headers on\n";
print FILE ".once gujcandidates2014/gujcandidates2014.csv\n";
print FILE "SELECT * FROM gujcandidates2014;\n";
