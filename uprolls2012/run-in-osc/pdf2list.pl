...
$dbh_rolls->do("ALTER TABLE rolls ADD COLUMN revision12 CHAR");
...
while (scalar(@raw)>0) {
	...
	if ($mode eq 'N') { # N - New Voter
		...
		if ($oldentry->{name} =~ /^$name/ and $oldentry->{fathername} eq $fathername) { # if name did not change
		    ...
		    $dbh_rolls->do ("UPDATE rolls SET voterid = ?, age = ?, gender = ?, revision12 = ? WHERE rollno = ?", undef, $voterid,  $age, $gender, 'M', $rollno);
	    ...
	    $dbh_rolls->do ("UPDATE rolls SET revision12 = ? WHERE rollno = ?", undef, $mode, $rollno);
...
system("./pdftotext -nopgbrk -f 1 -l 1 -layout -r 100 -x 139 -y 181 -W 193 -H 24 $file out.txt");
...
$dbh_rolls->do ("UPDATE booths SET lastupdate = ? WHERE booth = ?",undef,$raw,$booth);
