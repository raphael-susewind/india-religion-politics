...
our $lingo = 'name_bn'; # is it english or bangla?

# create hindi from english routine
sub hindi  {
    return '' if $_[0] eq '';
    hindi: my $result = $ua->get('http://www.google.com/transliterate/indic?tlqt=1&langpair=en|bn&text='.$_[0].'&&tl_app=1'); 
    if ($result->is_error && $result->error_code != 404) {print "Google transliterate down; retry\n"; sleep 5; goto hindi}
    my $json=$ua->content;
    $json=~/\:\s+\[\s+\"(.*?)\"/gs;
    return $1;
}
...
sub soundex {
    ...
    eval {
	    $timeout = 1;
  	 local $SIG{ALRM} = sub { die "alarm timeout on python\n" };
    	alarm $timeout;
...
# get arguments from command line
    ...
    if ($votername =~ /\p{Bengali}/) {$lingo = 'name_bn'} else {$lingo = 'name'} # switch english vs bengali
...
if (scalar(@files)<2) {system("rm $file");} # PDF corrupted / cannot be read
...
foreach my $cell (@cell) {
    ...
    my $gender='';
    if ($cell =~ /লিঙ্গ:\s+স্ত্রী /s or $cell =~ /FEMALE/) {$gender='f'} else {$gender='m'}

    my @content = split(/:/,$cell);
    
    my $name=$content[1];
    my @name = split(/\n/,$name); pop(@name); $name = join("",@name);
#    $name=~s/[^\p{Bengali}\s]//gs; 
    $name=~s/\s+/ /gs; 
    $name=~s/\s*$//gs;
    $name=~s/^\s*//gs;
...
if (scalar(@cell)>10) {
	...
	system("rm $constituency-$booth.pdf");
} else {
	system("rm $constituency-$booth-ocr.pdf");
}
