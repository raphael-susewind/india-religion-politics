...
system("echo 'create table andhragis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Andhra Pradesh\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE andhragis ADD COLUMN booth_id_21 INTEGER;' > andhragis2021.sql");
system("echo '.dump andhragis' | sqlite3 electoralsearch.in.sqlite >> andhragis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' andhragis2021.sql");
