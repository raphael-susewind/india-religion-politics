...
system("echo 'create table delhigis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Delhi\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE delhigis ADD COLUMN booth_id_21 INTEGER;' > delhigis2021.sql");
system("echo '.dump delhigis' | sqlite3 electoralsearch.in.sqlite >> delhigis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' delhigis2021.sql");
