...
$dbh->do ("CREATE TABLE andhraid (ac_id_09 INTEGER, booth_id_14 INTEGER, booth_name_14 CHAR, booth_address_14 CHAR, parts_14 CHAR, town_14 CHAR, thana_14 CHAR, mandal_14 CHAR, revenue_14 CHAR, district_14 CHAR, pincode_14 INTEGER)");
$dbh->do ("INSERT INTO andhraid (ac_id_09, booth_id_14, booth_name_14, booth_address_14, parts_14, town_14, thana_14, mandal_14, revenue_14, district_14, pincode_14) SELECT constituency, booth, trim(trim(replace(name,char(10),'-'),'-')), trim(trim(replace(address,char(10),'-'),'-')), trim(trim(replace(parts,char(10),'-'),'-')), trim(village), trim(thana), trim(mandal), trim(revenue), trim(district), pincode FROM booths");
