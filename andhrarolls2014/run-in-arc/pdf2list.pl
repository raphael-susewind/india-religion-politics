...
foreach my $cell (@xml) {
    $cell =~ s/<cell.*?\/>//gs;
    $cell =~ s/Photo of the//gs;
    $cell =~ s/elector is//gs;
    $cell =~ s/available//gs;
    $cell =~ s/\n/ /gs;
    
    next unless $cell =~ /Father/gs or $cell =~ /Husband/gs;
    ...
    my $gender='';
    if ($cell =~ /Female/) {$gender='f'} else {$gender='m'}
    ...
    if ($cell =~ /Husband/) { 
	$cell =~ /Name: (.*?)\s+Husband\'s Name: (.*?)\s+House/;
	$name=$1; $fathername=$2;
    } else {
	$cell =~ /Name: (.*?)\s+Father\'s Name: (.*?)\s+House/;
	$name=$1; $fathername=$2;
    }
...
#    unless ($shades > 10) { # not pure black & white, i.e. "deleted" overlay or some such
 ...
 #   system("rm $const-$booth-$rollno.tif");
