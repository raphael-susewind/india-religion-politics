...
sub devanagarilast {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name = ? AND namepart = 'l' GROUP BY community");
    $sth->execute($_[0]);
...
# match against devanagari first name gendered male
sub devanagarifirstm {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name = ? AND namepart = 'f' AND gender = 'm' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
# match against devanagari first name gendered female
sub devanagarifirstf {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name = ? AND namepart = 'f' AND gender = 'f' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
sub devanagarifirst {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name = ? AND namepart = 'f' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
system("camelot -p 3-end -f csv --output csvtemp lattice $file");
...
if (scalar(@cell)<50) {goto finalise} # sometimes camelot is fishy in which case we just exit
...
foreach my $cell (@cell) {
    ...
    $cell=~/^.*?([A-Z0-9\/ ][A-Z0-9\/ ][A-Z0-9\/ ][A-Z0-9\/ ].*?\d.*?)\n/s;
    my $voterid=$1;
    $voterid=~s/[^A-Z0-9\/]//gs;
      
    my $gender='';
    if ($cell =~ /FEMALE/s) {$gender='f'} elsif ($cell =~ /MALE/s) {$gender='m'}
	...
	$name=~s/\s*$//gs;
	$name=~s/^\s*//gs;
	if ($name =~ /Father/ or $name =~ /Husband/) {$name=''}
    }
	...
	$fathername=~s/\s*$//gs;
	$fathername=~s/^\s*//gs;
	if ($fathername =~ /House/) {$fathername=''}
    }
    
    $cell=~/.*[^\d](\d\d)[^\d]/gs;
    my $age=$1/1; 
    
    if ($gender eq '' && $age < 18) {print "Voter ID $voterid deleted; cell skipped\n\n"; next}  # these get garbled if delete stamp is is superimpoosed on cell
    if ($name eq '' && $fathername eq '') {print "Empty cell skipped\n\n"; next}
    
    if ($rollno > 1500 or $rollno == 0) {print "Cell has weird roll no?\nRoll No: $rollno\nVoter ID: $voterid\nGender: $gender, age: $age\nName: $name\nFathername: $fathername\n\n"; $rollno=''}
    if ($voterid !~ /\d\d\d\d/) {print "Cell has no voter id?\nRoll No: $rollno\nVoter ID: $voterid\nGender: $gender, age: $age\nName: $name\nFathername: $fathername\n\n"; $voterid=''}
