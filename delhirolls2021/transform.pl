...
system("sqlite3 temp.sqlite '.dump delhirolls2021' > delhirolls2021-a.sql");

open (FILE, ">>delhirolls2021-a.sql");

print FILE ".header on\n";
print FILE ".mode csv\n";
print FILE ".once delhirolls2021/delhirolls2021.csv\n";
print FILE "SELECT * FROM delhirolls2021;\n";

close (FILE);

system("sqlite3 temp.sqlite '.dump delhiid' > delhirolls2021-b.sql");

open (FILE, "delhirolls2021-b.sql");
my @file = <FILE>;
close (FILE);

open (FILE, ">delhirolls2021-b.sql");

print FILE "ALTER TABLE delhiid ADD COLUMN booth_id_21 INTEGER;\n";

foreach my $line (@file) {
    if ($line =~ /^INSERT INTO delhiid/) {$line =~ s/^INSERT INTO delhiid/INSERT INTO delhiid (ac_id_09, booth_id_21, booth_id_14)/; $line =~ s/'unclear'/NULL/gs; print FILE $line;}
}
