...
sub devanagarilast {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_kn = ? AND namepart = 'l' GROUP BY community");
    $sth->execute($_[0]);
...
# match against devanagari first name_kn gendered male
sub devanagarifirstm {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_kn = ? AND namepart = 'f' AND gender = 'm' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
# match against devanagari first name_kn gendered female
sub devanagarifirstf {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_kn = ? AND namepart = 'f' AND gender = 'f' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
sub devanagarifirst {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_kn = ? AND namepart = 'f' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
foreach my $cell (@xml) {
    ...
    next if $voterid =~ /cell/;
    
    my $gender='';
    if ($cell =~ /PhÂ\s+$/) {$gender='m'} else {$gender='f'}
...
#    my $age=$1;
 my $age='';
    ...
    system("tiffcrop -U px -z 155,58,550,190 $const-$booth-$rollno.tif $const-$booth-$rollno-ocr.tif");
...
#    unless ($shades > 10) { # not pure black & white, i.e. "deleted" overlay or some such
	...
	my $temp= `tesseract -psm 4 -l kan --tessdata-dir /home/area-mnni/rsusewind/share/tessdata $const-$booth-$rollno-ocr.tif stdout`;
	$temp =~ s/\n/ /gs;
        $temp =~ s/[^\p{Kannada}\s]//gs;

	my @temp=split(/\:/gs,$temp);
