...
system("echo 'create table goagis as select distinct ac \"ac_id_09\", booth \"booth_id_21\", latitude, longitude from electoralsearch where state = \"Goa\";' | sqlite3 electoralsearch.in.sqlite");
system("echo 'ALTER TABLE goagis ADD COLUMN booth_id_21 INTEGER;' > goagis2021.sql");
system("echo '.dump goagis' | sqlite3 electoralsearch.in.sqlite >> goagis2021.sql");
system("sed -i 's/VALUES/(ac_id_09, booth_id_21, latitude, longitude) VALUES/g' goagis2021.sql");
