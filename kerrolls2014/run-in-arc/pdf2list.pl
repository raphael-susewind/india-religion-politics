...
sub devanagarilast {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_ml = ? AND namepart = 'l' GROUP BY community");
    $sth->execute($_[0]);
...
# match against devanagari first name_ml gendered male
sub devanagarifirstm {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_ml = ? AND namepart = 'f' AND gender = 'm' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
# match against devanagari first name_ml gendered female
sub devanagarifirstf {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_ml = ? AND namepart = 'f' AND gender = 'f' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
sub devanagarifirst {
    my %result;
    my $sth = $dbh->prepare("SELECT community, count(*) 'count' FROM names WHERE name_ml = ? AND namepart = 'f' GROUP BY community");
    $sth->execute($_[0]);
    while (my $row=$sth->fetchrow_hashref) {$result{$row->{community}}=$row->{count};}
    $sth->finish ();
    return %result;
...
my $age='';
    ...
    system("tiffcrop -U px -z 240,35,800,120 $const-$booth-$rollno.tif $const-$booth-$rollno-ocr.tif");
...
#    unless ($shades > 10) { # not pure black & white, i.e. "deleted" overlay or some such
	...
	my $temp= `tesseract -psm 4 -l mal --tessdata-dir /home/area-mnni/rsusewind/share/tessdata $const-$booth-$rollno-ocr.tif stdout`;
        $temp =~ s/[^\p{Malayalam}\s]//gs;
