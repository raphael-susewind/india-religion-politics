...
$dbh_rolls->do("CREATE TABLE rolls (id INTEGER PRIMARY KEY AUTOINCREMENT, constituency INTEGER, booth INTEGER, nameparts INTEGER, age INTEGER, gender CHAR, community CHAR, certainty FLOAT, gap FLOAT, name CHAR, fathername CHAR, voter_nameparts INTEGER, voter_community CHAR, voter_certainty FLOAT, voter_gap FLOAT, father_nameparts INTEGER, father_community CHAR, father_certainty FLOAT, father_gap FLOAT, voterid CHAR, rollno INTEGER, ngram INTEGER, revision12 CHAR, revision13 CHAR, revision14 CHAR, revision15 CHAR, revision16 CHAR, revision17 CHAR, booth16 INTEGER)");

my $sth = $dbh_rolls->prepare("INSERT INTO rolls (constituency, rollno, voterid, nameparts, age, gender, community, certainty, gap, name, fathername, voter_nameparts, voter_community, voter_certainty, voter_gap, father_nameparts, father_community, father_certainty, father_gap, ngram, revision12, revision13, revision14, revision15, revision16, booth16) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
for ($page=3;$page<$pagecount;$page++) {
    $xml .= `PYTHONPATH=/home/area-mnni/rsusewind/lib/python2.6/site-packages:/system/software/linux-x86_64/lib/python2.6/site-packages python2.6 /home/area-mnni/rsusewind/bin/pdf-table-extract -i $file -p $page -r 300 -l 0.7 -t cells_xml`;
}
...
foreach my $cell (@xml) {
    ...
    my $revision17='N'; # setup revision variable
    
    # Now figure out if the voterid already exists
    $sth3->execute($voterid);
       
    while (my $row=$sth3->fetchrow_hashref) {
	$sth->execute($constituency, $rollno, $voterid, $row->{nameparts}, $row->{age}+1, $row->{gender}, $row->{community}, $row->{certainty}, $row->{gap}, $row->{name}, $row->{fathername}, $row->{voter_nameparts}, $row->{voter_community}, $row->{voter_certainty}, $row->{voter_gap}, $row->{father_nameparts}, $row->{father_community}, $row->{father_certainty}, $row->{father_gap}, $row->{ngram}, $row->{revision12}, $row->{revision13}, $row->{revision14}, $row->{revision15}, $row->{revision16}, $row->{booth});
	...
	my $fathername='';
	my $gender='';
	if ($cell =~ /पदरर\s/) {$gender='m'} else {$gender='f'}
	
	$cell=~s/\p{Devanagari}//gs;
	$cell =~ /:\s+(\d+)/gs;
	my $age=$1;
	...
	my $temp= `tesseract -psm 4 -l hin --tessdata-dir /home/area-mnni/rsusewind/share/tessdata $const-$booth-$rollno.tif stdout`;
	$temp =~ s/\n+/\n/gs;
...
if ($name.$fathername eq '') {system('rm *.tif'); next}
	...
	$dbh_rolls->do ("INSERT INTO rolls (constituency, booth, rollno, voterid, nameparts, age, gender, community, certainty, gap, name, fathername, voter_nameparts, voter_community, voter_certainty, voter_gap, father_nameparts, father_community, father_certainty, father_gap, revision17) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,  ?, ?, ?, ?, ?, ?, ?, ?)", undef, $constituency, $booth, $rollno, $voterid, $nameparts, $age, $gender, $community, $certainty, $percent,$name,$fathername,$voter_nameparts,$voter_community,$voter_certainty, $voter_percent,$father_nameparts,$father_community,$father_certainty,$father_percent,$revision17);
	my $id=$dbh_rolls->last_insert_id("","","","");
