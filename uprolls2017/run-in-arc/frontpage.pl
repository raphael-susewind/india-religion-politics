...
foreach my $file (@files) {
    ...
    $dbh->do("UPDATE booths SET name = ?, address = ?, parts = ?, village = ?, panchayat = ?, block = ?, thana = ?, tehsil = ?, district = ?, postoffice = ?, pincode = ? WHERE booth = ?",undef,$name,$address,$parts,$box[0],$box[1],$box[2],$box[3],$box[4],$box[5],$box[6],$pincode,$booth);
}
