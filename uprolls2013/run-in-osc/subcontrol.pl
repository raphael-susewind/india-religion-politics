...
system("cp /data/area-mnni/rsusewind/2012/$i/rolls*sqlite ."); # copy in old rolls against which changes are to be performed to arrive at new rolls
system("cp /data/area-mnni/rsusewind/2012/$i/ngram*lm ."); # copy in ngram profiles
...
foreach my $file (@files) {
    ...
    next if $temp =~ /revision13/;
    system("echo 'ALTER TABLE rolls ADD COLUMN revision13 CHAR;' | sqlite3 $file");
}
