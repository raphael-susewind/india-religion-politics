...
# Update age by one year for everybody - its a year later anyway...
$dbh_rolls->do("UPDATE rolls SET age = age + 1");

$dbh_rolls->do("ALTER TABLE rolls ADD COLUMN revision13 CHAR");

my $stx = $dbh_rolls->prepare("SELECT * FROM rolls WHERE id = ?");
...
while (scalar(@raw)>0) {
	...
	$first=shift(@raw);    
	if ($first !~ /^\s*([MNESRQ])\s+(\d+)\s+(.*?)\s*$/gs) {next}
	$mode=$1; $rollno=$2; $voterid = $3;
	
	repeat2:
	  $second=shift(@raw);
	goto repeat2 if $second !~ /\:/;
	$second=~/.*?\:\s*(.*?)\s*\n/gs; 
	$name=$1; $name=~s/;/,/gs;
	$name=~s/\.//gs;
	$name=~s/\s+/ /gs;
	...
	repeat4:
	  $fourth=shift(@raw);
	goto repeat4 if $fourth !~ /\:/;
	
	repeat5:
	  $fifth=shift(@raw);
	goto repeat5 if $fifth !~ /\:/;
	$fifth=~/.*?(\d+).*?\:\s*(.*?)\s*\n/gs;
	$age=$1; $gender=$2; 
	if ($age=~/[^0-9]/) {$age=''}
	$gender=~s/\x{093f}(.)/$1\x{093f}/gs; $gender=~s/;/,/gs;
	if ($gender =~ /^\x{092e}/) {$gender='f'}
	elsif ($gender =~ /^\x{092a}/) {$gender='m'}
	else {$gender=''}

	if ($mode eq 'N') { # N - New Voter
	    ...
	    $dbh_rolls->do ("INSERT INTO rolls (rollno, voterid, nameparts, age, gender, community, certainty, gap, name, fathername, voter_nameparts, voter_community, voter_certainty, voter_gap, father_nameparts, father_community, father_certainty, father_gap, revision13) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", undef, $rollno, $voterid, $nameparts, $age, $gender, $community, $certainty, $percent,$name,$fathername,$voter_nameparts,$voter_community,$voter_certainty, $voter_percent,$father_nameparts,$father_community,$father_certainty,$father_percent, 'N');
	...
	} elsif ($mode eq 'M') { # M - Modified Voter - run it again, its briefer that way
	    $stx->execute($rollno);
	    while (my $oldentry = $stx->fetchrow_hashref) {
		if ($oldentry->{name} =~ /^$name/ and $oldentry->{fathername} eq $fathername) { # if name did not change
		    
		    $dbh_rolls->do ("UPDATE rolls SET voterid = ?, age = ?, gender = ?, revision13 = ? WHERE rollno = ?", undef, $voterid,  $age, $gender, 'M', $rollno);
		    
		} else { # if name did change
		    $dbh_rolls->do ("DELETE FROM names WHERE id IN (SELECT id FROM rolls WHERE rollno = ?)", undef, $rollno);
		    ...
		    $dbh_rolls->do ("UPDATE rolls SET voterid = ?, nameparts = ?, age = ?, gender = ?, community = ?, certainty = ?, gap = ?, name = ?, fathername = ?, voter_nameparts = ?, voter_community = ?, voter_certainty = ?, voter_gap = ?, father_nameparts = ?, father_community = ?, father_certainty = ?, father_gap = ?, revision13 = ? WHERE rollno = ?", undef, $voterid, $nameparts, $age, $gender, $community, $certainty, $percent,$name,$fathername,$voter_nameparts,$voter_community,$voter_certainty, $voter_percent,$father_nameparts,$father_community,$father_certainty,$father_percent,'M', $rollno);
		    
		    foreach my $name (@$voterf) {$dbh_rolls->do("INSERT INTO names (id, firstname, soundex) VALUES (?, ?, ?)",undef,$rollno, $name, soundex($name));}
		    foreach my $name (@$voterl) {$dbh_rolls->do("INSERT INTO names (id, lastname, soundex) VALUES (?, ?, ?)",undef,$rollno, $name, soundex($name));}
		    foreach my $name (@$fatherf) {$dbh_rolls->do("INSERT INTO names (id, father_firstname, soundex) VALUES (?, ?, ?)",undef,$rollno, $name, soundex($name));}
		    foreach my $name (@$fatherl) {$dbh_rolls->do("INSERT INTO names (id, father_lastname, soundex) VALUES (?, ?, ?)",undef,$rollno, $name, soundex($name));}
		}
	    }
	    
	} else { # E - Dead Voter, S - Changed Adress, R - Duplicate
					  
	    $dbh_rolls->do ("UPDATE rolls SET revision13 = ? WHERE rollno = ?", undef, $mode, $rollno);
...
if ($file =~ /Supp-2/) {
    ...
    system("./pdftotext -nopgbrk -f 1 -l 1 -layout -r 100 -x 140 -y 181 -W 250 -H 25 $file out.txt");
    ...
    if ($raw =~ /\d/) {$dbh_rolls->do ("UPDATE booths SET lastupdate = ? WHERE booth = ?",undef,$raw,$booth);}
    
} elsif ($file =~ /Supp-3/) {

    system("./pdftotext -nopgbrk -f 1 -l 1 -layout -r 100 -x 138 -y 179 -W 250 -H 25 $file out.txt");
    open (FILE,"<:utf8","out.txt");
    my @raw=<FILE>;
    close (FILE);
    system("rm -f out.txt");
